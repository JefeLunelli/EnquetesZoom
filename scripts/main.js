...
window.addEventListener('load', function() {
  ...
  // Adicionar um evento de alteração ao elemento fileInput
  ...
  if (this.files.length > 0) {
    ...
    reader.onload = function (e) {
      ...
      // Encontrar a linha que começa com "Informação Assistência"
      var startIndex = lines.findIndex(line => line.startsWith('Informação Assistência'));
      startIndex += 2; // Pular a linha do cabeçalho

      for (var i = startIndex; i < lines.length; i++) {
        // Use vírgula como separador ao dividir a linha em células
        var row = lines[i].split(',');
        if (row.length < 5) break; // Parar quando chegar ao fim da seção "Informação Assistência"
